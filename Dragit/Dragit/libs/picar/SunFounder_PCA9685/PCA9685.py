...
class PWM(object):
    ...
    def _check_i2c(self):
        ...
        # Python 2: cmd = "ls /dev/i2c-%d" % bus_number
        # Python 2: output = commands.getoutput(cmd)
        cmd = subprocess.Popen(['ls', '/dev/i2c-%d' % bus_number], stdout=subprocess.PIPE, stderr=subprocess.PIPE)
        out, err = cmd.communicate()
        print('Commands "%s" output:' % cmd)
        print(out)
        if '/dev/i2c-%d' % bus_number in out.split(' '):
            print("I2C device setup OK")
        else:
            print("Seems like I2C have not been set, Use 'sudo raspi-config' to set I2C")
        # Python 2: cmd = "i2cdetect -y %s" % self.bus_number
        # Python 2: output = commands.getoutput(cmd)
        cmd = subprocess.Popen(['i2cdetect', '-y', self.bus_number], stdout=subprocess.PIPE, stderr=subprocess.PIPE)
        out, err = cmd.communicate()
