...
def cpu_temperature():
    # Python 2: raw_cpu_temperature = commands.getoutput("cat /sys/class/thermal/thermal_zone0/temp")
    cmd = subprocess.Popen(['cat', '/sys/class/thermal/thermal_zone0/temp'], stdout=subprocess.PIPE, stderr=subprocess.PIPE)
    out, err = cmd.communicate()
    cpu_temperature = float(out)/1000
    # cpu_temperature = 'Cpu temperature : ' + str(cpu_temperature)
    return cpu_temperature


def gpu_temperature():
    # Python 2: raw_gpu_temperature = commands.getoutput( '/opt/vc/bin/vcgencmd measure_temp' )
    cmd = subprocess.Popen(['/opt/vc/bin/vcgencmd measure_temp'], stdout=subprocess.PIPE, stderr=subprocess.PIPE)
    out, err = cmd.communicate()
    gpu_temperature = float(out.replace('temp=', '').replace('\'C', ''))
...
def get_i2c_address(busnum):
    # Python 2: cmd = "ls /dev/i2c-%d" % busnum
    cmd = subprocess.Popen(['ls', '/dev/i2c-%d' % busnum], stdout=subprocess.PIPE, stderr=subprocess.PIPE)
    out, err = cmd.communicate()
    print('Commands "%s" output:' % cmd)
    if '/dev/i2c-%d' % busnum in out.split(' '):
        print("I2C device setup OK")
    else:
        print("Seems like I2C have not been set, Use 'sudo raspi-config' to set I2C")
        return 'null'
    # Python 2: cmd = "i2cdetect -y %s" % busnum
    cmd = subprocess.Popen(['i2cdetect', '-y', busnum], stdout=subprocess.PIPE, stderr=subprocess.PIPE)
    out, err = cmd.communicate()
